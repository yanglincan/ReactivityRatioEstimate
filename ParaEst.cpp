...
void ParaEst::CalcOn_MCE()
{
	...
	WriteToFile(pFcalA_Vec_MC, "_MCE");		//输出数据
}
...
void ParaEst::CalcOn_M_L()
{
	...
	WriteToFile(pFcalA_Vec_ML, "_M-L");		//输出数据
}
...
void ParaEst::PostProba_YrVal(double stepSize_rA, double stepSize_rB, vector<rArBFAYr*>* pFcalA_V)
{
	...
	for (auto it_rArB = pFcalA_V->begin(); it_rArB != pFcalA_V->end(); it_rArB++){
		...
		(*it_rArB)->m_YrVal = yr;
		(*it_rArB)->m_DfVal = exp(-0.5 * yr);							//后验概率密度
		double uint = stepSize_rA * stepSize_rB;						//体积积分单元底面积dxdy
		cumulativeVolume += (*it_rArB)->m_DfVal * uint;					//体积积分累加
		rA_cal += (*it_rArB)->m_DfVal * (*it_rArB)->m_rA * uint;		//rA加权平均值
		rB_cal += (*it_rArB)->m_DfVal * (*it_rArB)->m_rB * uint;		//rB加权平均值
	}
	coefficient_C = TOTAL_PROBABILITY / cumulativeVolume;				//计算比例系数C，归一化处理。
